...
export default function TodoNameInput({ onClick, onClickAllTask }) {
    ...
    useEffect( () =>{
        document.querySelector('#add-task').addEventListener('keypress', handleKeyPressses);
      return () => {
        document.querySelector('#add-task').removeEventListener('keypress', handleKeyPressses);
      };
    });
